...
class XYWordMatch:
    ...
    def __init__(self, x0: float, x1: float, y0: float, y1: float, neighbour_tolerance: float = 0.75,
        ...
        self.short_text_and_number = set()
        self.text_for_word2vec = set()
        self.table_values = set()
    ...
    def add_short_text_and_number(self, text: str):
        self.short_text_and_number.add(text)

    def add_text_for_word2vec(self, text: str):
        self.text_for_word2vec.add(text)

    def add_table_values(self, text: str):
        self.table_values.add(text)
    ...
    def _calc_x_table_tolerance(self, x0: float, x1: float) -> tuple:
        ...
        """ Assuming that the value position is after the key position: """
        x0_lower = x0
        x1_upper = x1 + x_tolerance
        return x0_lower, x1_upper
...
class PDFMiner:
    ...
    def find_word_3(self, keywords: list, search_word_list: List[str], neighbour_tolerance: float,
                    table_tolerance: float, short_text_max_len: int = 10):
        findings = {'page_number': None, 'short_text_and_number': list(), 'text_for_word2vec': list(),
                                   'table_values': list()}
        page_number = 0
        ...
        for page in self.pages:
            ...
            # finding = {'page_number': None, 'short_text_and_number': list(), 'text_for_word2vec': list(),
            #            'table_values': list()}
            ...
            for first_layout_obj in page_layout:
                ...
                if isinstance(first_layout_obj, LTTextBox):
                    ...
                    if any(word in text_with_keyword for word in keywords):
                        ...
                        print(f'Page: {page_number}, text with search word: {text_with_keyword_clean}')
                        ...
                        for second_layout_obj in page_layout:
                            ...
                            if isinstance(second_layout_obj, LTTextBox):
                                if word_match.xx0_xx1_is_within_neighbour_bounds(xx0=xx0, xx1=xx1) or \
                                    ...
                                    if any(term.isdigit() for term in text_nearby_clean):
                                        ##############################################
                                        # print('text_with_keyword_clean:', text_with_keyword_clean)
                                        # print('text_nearby_clean:', text_nearby_clean)
                                        #########################################################
                            ...
                            if isinstance(second_layout_obj, LTTextLine):
                                if word_match.xx0_xx1_is_within_table_bounds(xx0=xx0, xx1=xx1):
                                    pass

                        finding['page_number'] = page_number
                        if word_match.short_text_and_number:
                            finding['short_text_and_number'].append(word_match.short_text_and_number)
                        if word_match.text_for_word2vec:
                            finding['text_for_word2vec'].append(word_match.text_for_word2vec)
                        if word_match.table_values:
                            finding['table_values'].append(word_match.table_values)
                # Finally, append all cleaned finding lists into another list ("findings"):
